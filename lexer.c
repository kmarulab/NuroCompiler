...
static void lex_finished_expression(){
    lex_process->current_expression_count--;
    if(lex_process->current_expression_count<0){
        compiler_error(lex_process->compiler,"You closed an expression you never opened\n");
    }
...
struct token* token_make_one_line_comment(){
    struct buffer* buffer = buffer_create();
    char c = 0;
    LEX_GETC_IF(buffer,c,(c != '/n') && (c != EOF));
...
struct token* token_make_multiline_comment(){
    ...
    while(1){
        ...
        if(c==EOF){
            compiler_error(lex_process->compiler,"You didn't close this multiline comment\n");
        } else if(c == '/'){
            nextc();
            if(peekc()=='/'){
                nextc();
